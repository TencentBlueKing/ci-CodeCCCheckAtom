...
object CodeccReportUtilsV2 {
    ...
    fun report(atomContext: AtomContext<CodeccCheckAtomParamV3>) {
        try {
            // 若是BS、GIT才生成产出报告，即流水线、stream
            val reportChannel = atomContext.getSensitiveConfParam("CODECC_REPORT_CHANNEL")?.split(",")?.map { it.trim() } ?: listOf()
            if (reportChannel.isNotEmpty() && atomContext.param.channelCode !in reportChannel) {
                LogUtils.printLog("report channel: $reportChannel")
                LogUtils.printLog("${atomContext.param.channelCode} do not get report...")
                return
            }

            val reportData = getReportData(
                atomContext.param.projectName,
                atomContext.param.codeCCTaskId!!,
                atomContext.param.pipelineBuildId,
                atomContext.result.data) ?: return
            val indexHtml = doReport(atomContext.param, reportData)
            atomContext.result.data["codecc_report"] = ReportData.createLocalReport(
                (atomContext.allParameters?.get("BK_CI_TASK_NAME")?.toString()
                    ?: I18NUtils.getMessage("asyncReport.label")),
            ...
            val codeccException = CodeCCBusinessException(
                ErrorCode.CODECC_REQUEST_FAIL,
                "request to codecc fail"
            )
    ...
    fun asyncReport(atomContext: AtomContext<CodeccCheckAtomParamV3>, detailLink: String) {
        try {
            ...
            atomContext.result.data["codecc_report"] = ReportData.createLocalReport(
                I18NUtils.getMessage("asyncReport.label"), tempAsyncDir.canonicalPath, indexHtml.name)
            // atomContext.result.data["codecc_resource"] = ArtifactData(setOf(chartOptionJs.canonicalPath, mainJs.canonicalPath, indexCss.canonicalPath))
        } catch (e: Exception) {
            LogUtils.printErrorLog(e.message, e)
        }
    }
    ...
    fun doAsyncReport(
        ...
        indexHtmlBody.append("<!DOCTYPE html>\n" +
            ...
            "        <meta charset=\"utf-8\"> \n" +
            "        <link rel=\"stylesheet\" href=\"${CodeccConfigUtils.getPropConfig("bkMagicVueCss")}\"> \n" +
            "        <link rel=\"stylesheet\" href=\"./${indexCss.name}\"> \n" +
            "        <title>${I18NUtils.getMessage("report.title")}</title> \n" +
            "    </head> \n" +
            "    <body> \n" +
            "        <div class=\"pipeline-code-check\" style=\"position: relative;\">\n" +
            "            <section class=\"devops-empty-tips\">\n" +
            "                <h2 class=\"title\">${I18NUtils.getMessage("asyncReport.title")}</h2>\n" +
            "                <p class=\"desc\">${I18NUtils.getMessage("asyncReport.desc")}</p>\n" +
            "                <p class=\"btns-row\">\n" +
            "                    <section>\n" +
            "                        <button title=\"\" class=\"bk-primary bk-button-normal bk-button\">\n" +
            "                            <a href=\"$detailLink\" target=\"_blank\">${I18NUtils.getMessage("asyncReport.hrefText")}</a> \n" +
            "                        </button>\n" +
            "                    </section>\n" +
            "                </p>\n" +
            "            </section>\n" +
    ...
    fun doReport(
        ...
        indexHtmlBody.append("<!DOCTYPE html>\n" +
            ...
            "        <script src=\"${CodeccConfigUtils.getPropConfig("bkMagicVueCss")}\"></script>\n" +
        ...
        indexHtmlBody.append("<div class=\"title-divider\">\n" +
            "    <span class=\"title-mark\"></span>\n" +
            "    <div class=\"divider-text\">${I18NUtils.getMessage("report.overview")}</div>\n" +
            "    <div class=\"row-head-link\">\n" +
            "        <a class=\"text-link\" href='${getTotalShowUrl(param.projectName, param.codeCCTaskId)}' target='_blank'>${I18NUtils.getMessage("report.label")}</a> \n" +
            "    </div>\n" +
            "</div>\n")
        indexHtmlBody.append("<div class=\"code-check-header-wrapper\">\n")

        val option = if (I18NUtils.currentLanguageIsZhCN()) {
            "codecc-options.json"
        } else {
            "codecc-options_en.json"
        }
        val json = BufferedReader(ClassLoader.getSystemClassLoader().getResourceAsStream(option).reader()).readText()
        ...
        reportData.toolSnapshotList.forEach {
            ...
            indexHtmlBody.append("<a class=\"code-check-head\" target='_blank' href='${getDefectDetailUrl(it["defect_detail_url"] as? String, param.channelCode, param.pipelineBuildId)}'>\n")
            val toolNameFinal = if (I18NUtils.currentLanguageIsZhCN()) toolNameCn else toolNameEn
            indexHtmlBody.append("<div class=\"code-check-title\">$toolNameFinal</div>\n")
        ...
        reportData.toolSnapshotList.groupBy {
            ...
            toolList.forEach { tool ->
                ...
                val codeccOptions = codeccOptionMap[toolNameEn] ?: getLintMap(toolNameCn)
                val displayToolName = if (I18NUtils.currentLanguageIsZhCN()) toolNameCn else toolNameEn
                indexHtmlBody.append("<section class=\"code-check-row lint-check-row\">\n" +
                    "                    <div class=\"row-head\">\n" +
                    "                        <div class=\"row-head-text\">$displayToolName</div>\n" +
                    "                        <div class=\"row-head-link\">\n" +
                    "                            <a class=\"text-link\" href='${getDefectDetailUrl(tool["defect_report_url"] as? String, param.channelCode, param.pipelineBuildId)}' target='_blank'>${I18NUtils.getMessage("report.label")}</a>\n" +
                    "                        </div>\n" +
                    "                    </div>\n" +
                ...
                indexHtmlBody.append("</div>\n")
//                if (toolTypeNameMap[toolNameEn]?.pattern == "LINT") indexHtmlBody.append("<p class=\"lint-tool-tips\">注：以上为起始时间后产生的新问题，鼓励该工具新问题清零。</p>\n")
                indexHtmlBody.append("</section>\n")
            }
        }
    ...
    private fun getDefectDetailUrl(originUrl: String?, channelCode: String?, buildId: String?): String {
        ...
        // stream
        if ("GIT" == channelCode) {
            val urlPath = "${URL(originUrl).path.removePrefix("/console")}?buildId=${buildId ?: ""}"
            val hostWithScheme = CodeccConfigUtils.getPropConfig("codeccDetail")

            return "$hostWithScheme$urlPath"
        }
    ...
    // 返回总览页面URL
    private fun getTotalShowUrl(projectName: String?, codeCCTaskId: String?): String {
...
//        val color = chart["color"] as List<String>
    ...
    private fun getLineJs(
        ...
        return "const $chartName = echarts.init(document.getElementById('$chartName'))\n" +
            ...
            "            ${chartName}Option.series[0].name = '$displayToolName'\n" +
    ...
    private fun getLintJson(): String {
        val option = if (I18NUtils.currentLanguageIsZhCN()) {
            "lint-codecc-options.json"
        } else {
            "lint-codecc-options_en.json"
        }
        return BufferedReader(ClassLoader.getSystemClassLoader().getResourceAsStream(option).reader()).readText()
    }
    ...
    private fun getReportData(projectId: String, taskId: String, buildId: String, outputData: MutableMap<String, DataField>): CodeccCallback? {
        for (i in (1..30)) {
            val callback = CodeccSdkApi.getCodeccReport(projectId, taskId, buildId)
            if (callback != null && callback.toolSnapshotList.isNotEmpty()) {
                LogUtils.printLog("get codecc report data success")
        ...
        LogUtils.printErrorLog("Failed to get report data")
        return null
    }
    ...
    fun getRedLineMetadataReportStatus(projectId: String, taskId: String, buildId: String): Result<Boolean?>? {
        for (i in (1..30)) {
            val callback = CodeccSdkApi.getRedLineMetadataReportStatus(projectId, taskId, buildId)
            if (callback?.data != null) {
                LogUtils.printLog("get redline metadata report callback success.")
                return callback
            }
            Thread.sleep(5000)
        }
        LogUtils.printErrorLog("Failed to upload CodeCC red line data!!")
        return null
    }
